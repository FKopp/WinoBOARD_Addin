...
void loop(void)
{
    ...
    if (wifi.connect("TCP",mux_id, HOST_NAME, HOST_PORT)) {
        SerialUSB.print("create tcp ");
    ...
    if (wifi.write(mux_id, (const uint8_t*)hello, strlen(hello))) {
        SerialUSB.println("send ok");
    } else {
        SerialUSB.println("send err");
    }
